...
#' dotsConnectText=TRUE,nameType="c",silent=T)
#'
#'
#' # Now let's shorten the gap between the tree and the text, only show common name,
#' # and increase text size
#' (g<-showPhylo(noms,"c",pic="n",dateTree=FALSE,phyloCol="royalblue1",dotsConnectText=TRUE,
#' nameType="c",labelOffset=.2,textScalar=1.2,silent=TRUE))
#'
...
#' theme(plot.title=element_text(size=18)) ,silent=TRUE)
#'
...
#' (g3<-showPhylo(noms[c(1,3,5,6,7)], "c" ,silent=TRUE))
